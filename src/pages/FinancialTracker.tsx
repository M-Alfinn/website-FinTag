  ...
  return (
    <motion.div 
      ...
      className={cn(
        ...
        isPrimary 
          ? "bg-secondary dark:bg-primary text-white border-white/10" 
          : isWarning 
            ? "bg-emerald-50 dark:bg-emerald-900/30 border-emerald-100 dark:border-emerald-800/30 text-slate-900 dark:text-emerald-400" 
            : "text-slate-900 dark:text-white"
      )}
    >
      <div className="flex justify-between items-start">
        <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center border", isPrimary ? "bg-white/10 border-white/10" : "bg-slate-50 dark:bg-slate-800 border-white dark:border-white/10 shadow-sm")}>
          <Icon className={cn("w-5 h-5", isPrimary ? "text-white" : "text-slate-400 dark:text-slate-300")} />
        </div>
        <span className={cn("text-[10px] font-bold uppercase tracking-[0.1em]", isPrimary ? "text-white" : isWarning ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-300")}>{trend}</span>
      </div>
      <div>
        <p className={cn("text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-100 mb-1")}>{label}</p>
